...
const ChartEnergyConsumptionPlantHungary = () => {
    const [chartData, setChartData] = useState({})

    useEffect(() => {
        const fetchData = async () => {
            const result = await fetch('http://localhost:8080/api/energyconsumption/hungary')
            ...
            setChartData({
                ...
                datasets: [
                    {
                        label: 'ENERGY CONSUMPTION PLANT HUNGARY',
                        data: data.data,
...
export default ChartEnergyConsumptionPlantHungary